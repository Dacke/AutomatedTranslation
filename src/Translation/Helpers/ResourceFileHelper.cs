...
namespace AutomatedTranslation.Helpers
{
    public class ResourceFileHelper : IDisposable, IResourceFileHelper
    {
        private readonly XDocument xDocSource;
        private XDocument xDocTarget;
        ...
        public ResourceFileHelper(String englishResourceFile, String targetResourceFile)
        {
            ...
            xDocSource = XDocument.Load(EnglishResourceFile);
            xDocTarget = XDocument.Load(TargetResourceFile);
        }

        public void Dispose()
        {
            try { xDocTarget.Save(TargetResourceFile); } 
            catch (Exception ex) { throw ex; }
        }
        
        public Dictionary<String, String> GetNameValuesFromSource(string value)
        {
            return GetNameValueDictionaryFromResourceFile(xDocSource, value);
        }

        public Dictionary<String, String> GetAllNameValuesFromSource()
        {
            return GetAllValuesDictionaryFromResourceFile(xDocSource);
        }

        public Dictionary<String, String> GetNameValuesFromTargetUsingValue(string value)
        {
            return GetNameValueDictionaryFromResourceFile(xDocTarget, value);
        }

        public Dictionary<String, String> GetAllNameValuesFromTarget()
        {
            return GetAllValuesDictionaryFromResourceFile(xDocTarget);
        }

        public String GetValueFromTargetUsingKey(string key)
        {
            var value = GetXElementForKeyOrDefault(xDocTarget, key).Element(VALUE_ELEMENT).Value;
            return value;
        }
        
        public void WriteNameValuePairToTarget(string key, string value, bool overwrite)
        {
            var targetValue = GetXElementForKeyOrDefault(xDocTarget, key);
        ...
        {
            xDocTarget.Save(this.TargetResourceFile);
            xDocTarget = XDocument.Load(TargetResourceFile);
        }
